...
var get = this.get = function(path) {
  var obj = world,
      i, prop, ref, key;
  if (path) {
    path = path.split('.');
...
var _set = this._set = function(path, value, silent, sendUpdate, broadcast) {
  var obj = world,
      eventPath = [],
...
var set = this.set = function(path, value, broadcast) {
  _set(path, value, false, true, broadcast);
};
var setSilent = this.setSilent = function(path, value) {
  _set(path, value, true);
};

var _push = this._push = function(name, value, sendUpdate, broadcast) {
  var arr = world[name];
  arr.push(value);
  events.trigger(name, arr);
  if (sendUpdate) send('push', [name, value], broadcast);
};
var push = this.push = function(name, value, broadcast) {
  _push(name, value, true, broadcast);
};

this.ref = function(ref, key) {
